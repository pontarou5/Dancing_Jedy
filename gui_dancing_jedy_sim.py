...
# Gazebo起動〜コントローラが動かせる状態になるまでの目安の待ち時間。
# マシンによっては足りない/長すぎる場合があるので必要に応じて調整すること。
...
def setup_dance_scripts(music_name):
    ...
    subprocess.Popen(["gnome-terminal", "--", "bash", "-c", "roseus jedy_dance_subscribe.l; exec bash"])
    subprocess.Popen(["gnome-terminal", "--", "bash", "-c", "roseus wheel_move_dance.l; exec bash"])
    # ダンス準備が完了したので「ダンススタート」を押せるようにする
    _set_start_button_ready(True)

def prepare_dance(music_name):
    """ROSの起動とダンススクリプトの準備をまとめて行う"""
    # 準備が完了するまでは「ダンススタート」を押せないようにする
    _set_start_button_ready(False)
    print("新しいターミナルで jedy_gazebo_fullbody_modified.launch を実行")
    subprocess.Popen(["gnome-terminal", "--", "bash", "-c", "roslaunch jedy_bringup jedy_gazebo_fullbody_modified.launch; exec bash"])
    # Gazebo/コントローラが立ち上がりきる前にダンススクリプトを動かすと失敗するので、
    # 少し待ってから setup_dance_scripts を呼ぶ（GUIをブロックしないよう root.after で遅延）
    root.after(GAZEBO_STARTUP_DELAY_MS, lambda: setup_dance_scripts(music_name))
...
def stop_music_and_ros():
    """音楽とROS(ダンス/ホイールスクリプト含む)をまとめて停止し、次の曲のために状態をクリアする"""
    print("音楽とROSを停止")
